...
public class Melangeur {
    ...
    public Tuile[][] genererNouveauPlateau() {
        ArrayList<Coordonnees> emplacementPossible = new ArrayList<>();
        ...
        for (int indexLigne = 1; indexLigne < Plateau.NOMBRE_LIGNE - 1; indexLigne++) {
            for (int indexColonne = 1; indexColonne < Plateau.NOMBRE_COLONNE - 1; indexColonne++) {
                if (emplacementDansTerrain(indexLigne, indexColonne)) {
                    emplacementPossible.add(new Coordonnees(indexLigne, indexColonne));
                }
            }
        }
    ...
    public Object[] melangerPlateau(Tuile[][] plateau, ArrayList<Tuile> tuileEnJeu, long seed) {
        ArrayList<Coordonnees> emplacementPossible = genererTableauEmplacementPossible(plateau);
     ...
     */
    private Tuile[][] genererPlateau(ArrayList<Coordonnees> emplacementPossible, ArrayList<Tuile[]> listeDePaires) {
        Random random = new Random(seed);
        ArrayList<Coordonnees> emplacementDisponible = new ArrayList<>();
     ...
     */
    private ArrayList<Coordonnees> genererTableauEmplacementPossible(Tuile[][] plateau) {
        ArrayList<Coordonnees> emplacementPossible = new ArrayList<>();
     ...
     */
    public void mettreAJourEmplacementDisponible(Coordonnees coordonnee, Random random,
            ArrayList<Coordonnees> emplacementDisponible,
            ArrayList<Coordonnees> emplacementPossible) {
        if (!emplacementPossible.isEmpty()) {

            for (Direction caseAdjacente : Direction.values()) {
                Coordonnees nouvelleCoordonnee = new Coordonnees(
